...
public class LaunchSparkTask extends DefaultTask {
    ...
    public void launch() throws IOException {
        ...
        String sparkConf = settings.getSparkConfFilePath();
        SPGLogger.logInfo.accept(String.format("Spark conf properties file at %s", sparkConf));
        String dockerImageId = settings.getDockerImageTag();
        SPGLogger.logInfo.accept(String.format("Docker Image id to be used at Driver %s", dockerImageId));

        final List<Object> prgArgs = asList(new Object[]{settings.getAppName(), settings.getMaster(),
                files[0].toPath().toString(), mainClass, errFile, outFile, sparkHome, sparkConf, dockerImageId});
        ...
        p.javaexec(new Action<JavaExecSpec>() {
            ...
            public void execute(JavaExecSpec spec) {
                ...
                } else if (runMode == SparkRunMode.K8S_CLUSTER) {
                    SPGLogger.logFine.accept(PROPERTY_SET_VALUE.apply(SPARK_CONF_DEPLOY_MODE, settings.getMode()));
                    spec.getEnvironment().put(SPARK_CONF_DEPLOY_MODE, settings.getMode());
                }
